...
class CServiceAdmin extends CSectionAdmin{

	var $table = "services";
	var $actions = array("edit","clone", "delete");
	var $mItemsPerPage = 40;
	var $mLabels = array("Services", "Service");
	var $mClass = "CService";
  ...
  function display() {
	...
	$vSmart->addHeader(array("Order", "Image", "Title"));
	$vSmart->addEditField("OrderID");
	$vSmart->addImageField("Image", "100px");
	$vSmart->addEditField("Name");

	$vSmart->mDefaultOrder = "a.OrderID";
	$vSmart->mDefaultOrderDir = "ASC";
	
	$vSmart->addCompositeFilter("Content", "a.Name, a.Summary", "Search", 1, "input_search w400");
...
//	$vSmart->addExtraActions(new CHref(getLink() . "order", "Mass Sort"));
	...
	$vSmart->mColsWidths = array("20px", "60px",  "100px", "100px", "60%", "60px");
  ...
  function displayPreview($id){
	  $news = new CService($id, $this->table);
