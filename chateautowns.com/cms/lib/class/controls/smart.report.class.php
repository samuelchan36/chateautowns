...
class CSmartReport extends CHtmlEntity {
...
//	die2($pAction);
  ...
  /** comment here */
  function addGroupBy($grouping) {
	$this->mGroupBy = $grouping;
  }

  /** comment here */
  function addSmartFields($fields) {
	$this->mSmartFields = $fields;
  }
...
//	  die2($_GET);
	...
	$this->template = template2("lib/html/report.html");
//	$this->newBlock("MAIN");
...
//	die($criteria);
	...
	#create sql
	if (!isset($_GET["sr_GroupBy"]) || !$_GET["sr_GroupBy"]) $groupby = 0;
	else {
		$groupby = $_GET["sr_GroupBy"];
		$_SESSION["gAdminFilters"]["sr_GroupBy"] = $_GET["sr_GroupBy"];
	}

	if (!isset($_GET["sr_OrderBy"])) {
		$tmp1 = explode(",", $this->mGroupBy[$groupby][1]);
		$orderby = array();
...
//		die($orderby);
			...
			$_SESSION["gAdminFilters"]["sr_OrderBy"] = $_GET["sr_OrderBy"];
			switch($_GET["sr_OrderBy"]) {
				case "sum": $orderby = "count(".$this->mSmartFields[0] .") "; break;
				case "avg": $orderby = "sum(".$this->mSmartFields[1] .")/count(".$this->mSmartFields[0] .") "; break;
				case "orderid": $orderby = $this->mGroupBy[$groupby][0] . " "; break;
				default: 
					$tmp1 = explode(",", $this->mGroupBy[$groupby][1]);
					$orderby = array();
					foreach ($tmp1 as $key=>$val) {
	...
	if (!isset($_GET["sr_OrderDir"])) $orderdir  = " DESC ";  else {
		$orderdir = $_GET["sr_OrderDir"];
		$_SESSION["gAdminFilters"]["sr_OrderDir"] = $_GET["sr_OrderDir"];
	}

	$this->mSql = "SELECT ".$this->mGroupBy[$groupby][1] ." as Label, count(".$this->mSmartFields[0] .") as ItemCount, sum(".$this->mSmartFields[1] .") as ItemSum, sum(".$this->mSmartFields[1] .")/count(".$this->mSmartFields[0] .") as ItemAverage " .   
								str_replace("##CRITERIA##",$criteria,$this->mSql) . " 
								GROUP BY ".$this->mGroupBy[$groupby][0] . "
...
//	die($this->mSql);
	...
	$items = $this->mDatabase->getAll($this->mSql);
	$rows = array();
	
	$tmp = explode(",", $this->mGroupBy[$groupby][1]);
	$tmp2 = explode(",", $this->mGroupBy[$groupby][2]);
	$labels = count($tmp) -1;
	$sum = 0;
	$cnt= 0;
	foreach ($items as $key=>$val) {
		for($i=1; $i<=$labels; $i++) {
			$rows[$key+1][] = $val["Label$i"];
		}
		$sum += $val["ItemSum"];
		$cnt += $val["ItemCount"];
		$rows[$key+1][] = $val["Label"];
		$rows[$key+1][] = number_format($val["ItemCount"], 0, ".", ",");
		$rows[$key+1][] = number_format($val["ItemAverage"], 4);
	}
	$rows[$key+2][] = "<b>Total</b>";
	for($i=1; $i<=$labels; $i++) {
		$rows[$key+2][] = "&nbsp;";
	}
	$rows[$key+2][] = "<b>" . number_format($cnt, 0, ".", ",") . "</b>";
	$rows[$key+2][] = "<b>" . number_format($sum / $cnt, 2) . "</b>";
	...
	$aligns = array();
	for($i=1; $i<=$labels; $i++) {
		$header[] = "<span>".$tmp2[$i-1]."</span>";;
		$widths[] = array(80/($labels +1) . "%");
		$aligns[] = "left";
	}	
	$widths[] = array(80/($labels +1) . "%");
	$aligns[] = "left";
	$header[] = "<span>".$tmp2[count($tmp2)-1]."</span>";
	...
	$footer = array();
	  ...
	  $vTable->mColsAligns = $aligns;
	  $vTable->mColsWidths = $widths;
...
//die2($this->mFilters);
	foreach ($this->mFilters as $key=>$val) {
	  foreach ($val as $key2=>$val2) {
		$rows[] ="<div class='filter-cell filter-".$val2[3]."' style=''>" . $val2[0] . "</div>";
	  }
	}
		$options = array();
		foreach ($this->mGroupBy as $key=>$val) {
			$options[] = array($key, $val[2]);
		}
//		die2($_GET);
		$input = new CSelect("sr_GroupBy", $options);
		if (isset($_GET["sr_GroupBy"])) $input->value = $_GET["sr_GroupBy"];
		 $rows2[] .= "<div class='filter-cell  style=''>" . $input->display() . "</div>";
		$input = new CSelect("sr_OrderBy", array(array("label", "Label"), array("orderid", "Order ID"), array("count", "Number of Items"), array("avg", "Average")));
		if (isset($_GET["sr_OrderBy"])) $input->value = $_GET["sr_OrderBy"];
		 $rows3[] .= "<div class='filter-cell  style=''>" . $input->display() . "</div>";
		$input = new CSelect("sr_OrderDir", array(array("desc", "Descending"), array("asc", "Ascending")));
		if (isset($_GET["sr_OrderDir"])) $input->value = $_GET["sr_OrderDir"];
		 $rows3[] .= "<div class='filter-cell  style=''>" . $input->display() . "</div>";


//	$vTable = new CGrid($rows, array(), "");
//	$vTable->setTemplate("filters");
	$txt = "<span class='filter-group-label'>FILTERS</span> " . implode("", $rows)  . 
				"<span class='filter-group-label'>GROUP BY</span> " . implode("", $rows2)  . 
				"<span class='filter-group-label'>ORDER BY</span> " . implode("", $rows3)  .
				$this->addParams($this->mSection, $this->mOperation);
	$txt .= '<div class="filter-actions"><a id="bt-reset-filters" title="Reset Filters"><i class="fas fa-ban"></i></a> <a id="bt-apply-filters"  title="Apply Filters"><i class="fas fa-check-circle"></i></a></div>';
