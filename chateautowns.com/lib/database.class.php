...
class CDatabase {
	...
	function connect() {
		...
		try {
			if (!mysqli_select_db($this->mConnection, $this->mDb)) $this->initDB();;
		} catch (Exception $ex) {
			$this->initDB();
		}
	...
	/** comment here */
	function initDB() {
			die("Database doesn't exist, <a href='/initdb.php'>click here</a> to initialize");
	...
	function execute($pSql) {
		...
		if (LOG_QUERIES == "yes" && $pSql) mysqli_query($this->mConnection, "insert into cms_log_queries(TimeStamp, Response, Query) values(unix_timestamp(), ".($vResult ? 1 : 0).", '".mysqli_real_escape_string($this->mConnection, $pSql)."')");
...
//		die2($pDataAry);
	...
	function getMax($pTable, $pField, $pCond){
		$vTmp = mysqli_list_fields($this->mDb,$pTable,$this->mConnection);
